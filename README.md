...
형태소 분석을 하지 않으면 `디즈니` 와 `디즈니는`, `디즈니와` 가 각각 다른 단어로 인식되어 버리는군요.
...
`soy` 는 띄어쓰기 통계기반으로 형태소를 판단하는데 꽤 괜찮은 결과를 보이네요.
...
사용자사전을 많이 등록해야만 올바른 결과가 나오는 듯 합니다.
...
사전파일만 16M 가 넘어서 그런지 꽤 좋은 결과가 나오는군요. 그 와중에 `지브리` 가 나오네요.
